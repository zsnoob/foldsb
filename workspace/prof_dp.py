...
Multi-node:
    Node 0 (assume the node has ip of 10.99.48.128):
            python modified_data_parallel.py \
                    --model="../scripts/DeepSeek-V3" \
                    --dp-size=2 \
                    --tp-size=2 \
                    --node-size=2 \
...
def main(model, dp_size, local_dp_rank, global_dp_rank, dp_master_ip,
         dp_master_port, GPUs_per_dp_rank, input_file=None):
    ...
    # Sample prompts or read from file
    ...
    sampling_params = SamplingParams(temperature=0.8,
                                     top_p=0.95,
                                     max_tokens=16 * (global_dp_rank + 1))
